defmodule Elixdo.Lists.DB do
  ...
  def create_items(date, items_attrs) when is_list(items_attrs) do
    max_pos = Repo.one(from i in ListItem,
      where: i.date == ^date,
      select: max(i.position)) || 0
  ...
  def update_item(%ListItem{} = item, attrs) do
    case Map.get(attrs, :status) || Map.get(attrs, "status") do
      ...
      new_status when is_binary(new_status) ->
        normalized = attrs |> Map.delete("status") |> Map.put(:status, String.to_existing_atom(new_status))
  ...
  def arrow_item(%ListItem{status: :active} = item, to_date) do
    ...
    |> Ecto.Multi.run(:copy, fn _repo, _changes ->
      create_items(to_date, [%{
        body: item.body, color: item.color,
        bold: item.bold, italic: item.italic,
        highlighted: item.highlighted, prefix: item.prefix
      }])
    end)
    |> Repo.transaction()
    |> case do
      {:ok, %{original: original, copy: [copy]}} -> {:ok, original, copy}
      {:ok, %{original: original, copy: copy}} when is_list(copy) -> {:ok, original, List.first(copy)}
